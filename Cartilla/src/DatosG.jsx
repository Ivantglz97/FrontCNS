...
import './DatosG.css'; // Para los estilos personalizados
          ...
          <div className="right-column">
            ...
            <div className="form-group">
              ...
              <input
                ...
                value={userData ? userData.fechaNacimiento : ''}
                readOnly
              />
            </div>
          </div>

          {/* Lector de QR al lado del formulario */}
          <div className="qr-container">
            <h3>Escanear QR</h3>
            <video ref={videoRef} style={{ width: '100%' }} />
            {qrData && (
              <div className="qr-result">
                <h4>QR leído: {qrData}</h4>
              </div>
            )}
            {/* Botón para apagar la cámara */}
            {isCameraActive && (
              <button type="button" onClick={stopScanner} className="stop-camera-button">
                Apagar Cámara
              </button>
            )}
            {!isCameraActive && (
              <button type="button" onClick={startScanner} className="start-camera-button">
                Iniciar Cámara
              </button>
            )}
          </div>
